...
jsPsych.plugins['survey-template'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      scale2: {
        ...
        pretty_name: 'Scale 2',
        default: [],
        decription: 'Alternate response options associated with the survey'
      },
      scale2_indices: {
        type: jsPsych.plugins.parameterType.HTML_STRING,
        array: true,
        pretty_name: 'Use Scale 2',
        default: [],
        decription: 'Indices of items for which scale 2 is used'
      },
      reverse: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.items.length; i++) {
      ...
      if (i % trial.scale_repeat == 0 || i == trial.scale2_indices[0]) {
        html += '<div class="survey-template-header"></div>';
        for (var j = 0; j < trial.scale.length; j++) {
          var scale_to_use = trial.scale2_indices.includes(i) ? trial.scale2 : trial.scale;
          html += `<div class="survey-template-header">${scale_to_use[j]}</div>`;
        }
      }
    ...
    display_element.querySelector('#survey-template-submit').addEventListener('submit', function(event) {
        ...
        // Extract responses
        var responses = JSON.stringify(question_data);
